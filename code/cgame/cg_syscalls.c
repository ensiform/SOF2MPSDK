...
{
	Q_syscall( CG_CVAR_REGISTER, vmCvar, varName, defaultValue, flags, PASSFLOAT(MinValue), PASSFLOAT(MaxValue) );
}
...
{
	Q_syscall( CG_S_ADDLOOPINGSOUND, entityNum, origin, velocity, PASSFLOAT(radius), sfx );
}

void	trap_S_AddRealLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, float radius, sfxHandle_t sfx ) 
{
	Q_syscall( CG_S_ADDREALLOOPINGSOUND, entityNum, origin, velocity, PASSFLOAT(radius), sfx );
}
...
{
	Q_syscall( CG_AS_ADDPRECACHEENTRY, set );
}
...
{
	Q_syscall( CG_AS_UPDATEAMBIENTSET, name, origin );
}

int	trap_AS_AddLocalSet(const char *name, vec3_t listener_origin, vec3_t origin, int entID, int time)
{
	return Q_syscall( CG_AS_ADDLOCALSET, name, listener_origin, origin, entID, time );
}

sfxHandle_t	trap_AS_GetBModelSound(const char *name, int stage)
{
	return (sfxHandle_t)Q_syscall( CG_AS_GETBMODELSOUND, name, stage );
}
...
{
	return (qhandle_t)Q_syscall(CG_R_REGISTERFONT, fontName );
}
...
{
	Q_syscall( CG_R_DRAWVISUALOVERLAY, type, preProcess, PASSFLOAT(parm1), PASSFLOAT(parm2) );
}
...
void	trap_R_DrawStretchPic( float x, float y, float w, float h, 
	...
	Q_syscall( CG_R_DRAWSTRETCHPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), color, hShader );
}
...
{
	return Q_syscall( CG_R_GETTEXTWIDTH, text, font, PASSFLOAT(scale), limit );
}
	
int		trap_R_GetTextHeight ( const char* text, qhandle_t font, float scale, int limit )
{
	return Q_syscall( CG_R_GETTEXTHEIGHT, text, font, PASSFLOAT(scale), limit );
}

void	trap_R_DrawText ( int x, int y, qhandle_t font, float scale, vec4_t color, const char* text, int limit, int flags )
{
	Q_syscall( CG_R_DRAWTEXT, x, y, font, PASSFLOAT(scale), color, text, limit, flags );
}
...
{
	Q_syscall( CG_R_DRAWTEXTWITHCURSOR, x, y, font, PASSFLOAT(scale), color, text, limit, flags, cursorPos, cursor );
}
...
{
	Q_syscall( CG_RW_SETTEAM, team, dead );
}

void trap_ResetAutorun ( void )
{
	Q_syscall( CG_RESETAUTORUN );
}
...
{
	Q_syscall( CG_FX_PLAY_SIMPLE_EFFECT, file, org, vol, rad );
}
...
{
	Q_syscall( CG_FX_PLAY_SIMPLE_EFFECT_ID, id, org, vol, rad );
}
...
{
	Q_syscall( CG_FX_PLAY_BOLTED_EFFECT_ID, id, obj, vol, rad);
}

void trap_FX_AddScheduledEffects( void )
{
	Q_syscall( CG_FX_ADD_SCHEDULED_EFFECTS );
}
...
{
	Q_syscall( CG_G2_COLLISIONDETECT, collRecMap, ghoul2, angles, position, frameNumber, entNum, rayStart, rayEnd, scale, traceFlags, useLod );
}

int	trap_G2API_AddBolt(void *ghoul2, const int modelIndex, const char *boneName)
{
	return (int) (Q_syscall(CG_G2_ADDBOLT, ghoul2, modelIndex, boneName));
}
...
{
	return (qboolean)(Q_syscall(CG_G2_REMOVEBOLT, ghlInfo, modelIndex, index));
}

qboolean trap_G2API_AttachG2Model(void *ghoul2From, int modelFrom, void *ghoul2To, int toBoltIndex, int toModel)
{
	return (qboolean)(Q_syscall(CG_G2_ATTACHG2MODEL, ghoul2From, modelFrom, ghoul2To, toBoltIndex, toModel));
}

qboolean	trap_G2API_DetachG2Model(void *ghoul2, int modelIndex)
{
	return (qboolean)(Q_syscall(CG_G2_DETACHG2MODEL, ghoul2, modelIndex));
}
...
{
	return Q_syscall(CG_G2_GETANIMFILENAMEINDEX, ghoul2, modelIndex, filename );
}

qhandle_t trap_G2API_RegisterSkin ( const char *skinName, int numPairs, const char *skinPairs)
{
	return Q_syscall(CG_G2_REGISTERSKIN, skinName, numPairs, skinPairs );
}

qboolean trap_G2API_SetSkin ( TGhoul2 ghoul2, int modelIndex, qhandle_t customSkin)
{
	return Q_syscall(CG_G2_SETSKIN, ghoul2, modelIndex, customSkin );
}
...
{
	return Q_syscall(CG_G2_GETANIM, ghoul2, modelIndex, boneName, currentTime, frame );
}

qboolean trap_G2API_SetSurfaceOnOff(void *ghoul2, const int modelIndex, const char *surfaceName, const int flags)
{
	return Q_syscall(CG_G2_SETSURFACEONOFF, ghoul2, modelIndex, surfaceName, flags);
}
...
{
	return Q_syscall(CG_G2_SETNEWORIGIN, ghoul2, modelIndex, boltIndex);
}

char *trap_G2API_GetGLAName(void *ghoul2, int modelIndex)
{
	return (char *)Q_syscall(CG_G2_GETGLANAME, ghoul2, modelIndex);
}
...
{
	return Q_syscall(CG_G2_COPYSPECIFICGHOUL2MODEL, g2From, modelFrom, g2To, modelTo);
}
...
{
	return(Q_syscall(CG_G2_SETGHOUL2MODELFLAGS,ghlInfo,flags));
}

int trap_G2API_GetGhoul2ModelFlags(void *ghlInfo)
{
	return(Q_syscall(CG_G2_GETGHOUL2MODELFLAGS,ghlInfo));
}

qboolean trap_G2API_SetGhoul2ModelFlagsByIndex(void *ghoul2,int index,int flags)
{
	return(Q_syscall(CG_G2_SETGHOUL2MODELFLAGSBYINDEX,ghoul2,index,flags));
}

int trap_G2API_GetGhoul2ModelFlagsByIndex(void *ghoul2,int index)
{
	return(Q_syscall(CG_G2_GETGHOUL2MODELFLAGSBYINDEX,ghoul2,index));
}

int trap_G2API_GetNumModels(TGhoul2 ghoul2)
{
	return Q_syscall(CG_G2_GETNUMMODELS, ghoul2);
}

int trap_G2API_FindBoltIndex(TGhoul2 ghoul2, const int modelIndex, const char *boneName)
{
	return Q_syscall(CG_G2_FINDBOLTINDEX, ghoul2, modelIndex, boneName);
}

int trap_G2API_GetBoltIndex(TGhoul2 ghoul2, const int modelIndex)
{
	return Q_syscall(CG_G2_GETBOLTINDEX, ghoul2, modelIndex);
}

void trap_MAT_Init(void)
{
	Q_syscall(CG_MAT_CACHE);
}

void trap_MAT_Reset(void)
{
	Q_syscall(CG_MAT_RESET);
}

sfxHandle_t trap_MAT_GetSound(char *key, int material)
{
	return (sfxHandle_t)Q_syscall(CG_MAT_GET_SOUND, key, material);
}

qhandle_t trap_MAT_GetDecal(char *key, int material)
{
	return (sfxHandle_t)Q_syscall(CG_MAT_GET_DECAL, key, material);
}

const float trap_MAT_GetDecalScale(char *key, int material)
{
	return (const float)Q_syscall(CG_MAT_GET_DECAL_SCALE, key, material);
}

qhandle_t trap_MAT_GetEffect(char *key, int material)
{
	return (sfxHandle_t)Q_syscall(CG_MAT_GET_EFFECT, key, material);
}

qhandle_t trap_MAT_GetDebris(char *key, int material)
{
	return (sfxHandle_t)Q_syscall(CG_MAT_GET_DEBRIS, key, material);
}

const float trap_MAT_GetDebrisScale(char *key, int material)
{
	return (const float)Q_syscall(CG_MAT_GET_DEBRIS_SCALE, key, material);
}

// CGenericParser2 (void *) routines
TGenericParser2 trap_GP_Parse(char **dataPtr, qboolean cleanFirst, qboolean writeable)
{
	return (TGenericParser2)Q_syscall(GP_PARSE, dataPtr, cleanFirst, writeable);
}

TGenericParser2 trap_GP_ParseFile(char *fileName, qboolean cleanFirst, qboolean writeable)
{
	return (TGenericParser2)Q_syscall(GP_PARSE_FILE, fileName, cleanFirst, writeable);
}

void trap_GP_Clean(TGenericParser2 GP2)
{
	Q_syscall(GP_CLEAN, GP2);
}

void trap_GP_Delete(TGenericParser2 *GP2)
{
	Q_syscall(GP_DELETE, GP2);
}

TGPGroup trap_GP_GetBaseParseGroup(TGenericParser2 GP2)
{
	return (TGPGroup)Q_syscall(GP_GET_BASE_PARSE_GROUP, GP2);
}
...
{
	return (qboolean)Q_syscall(GPG_GET_NAME, GPG, Value);
}

TGPGroup trap_GPG_GetNext(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_NEXT, GPG);
}

TGPGroup trap_GPG_GetInOrderNext(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_INORDER_NEXT, GPG);
}

TGPGroup trap_GPG_GetInOrderPrevious(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_INORDER_PREVIOUS, GPG);
}

TGPGroup trap_GPG_GetPairs(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_PAIRS, GPG);
}

TGPGroup trap_GPG_GetInOrderPairs(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_INORDER_PAIRS, GPG);
}

TGPGroup trap_GPG_GetSubGroups(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_SUBGROUPS, GPG);
}

TGPGroup trap_GPG_GetInOrderSubGroups(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(GPG_GET_INORDER_SUBGROUPS, GPG);
}

TGPValue trap_GPG_FindSubGroup(TGPGroup GPG, const char *name)
{
	return (TGPValue)Q_syscall(GPG_FIND_SUBGROUP, GPG, name);
}

TGPValue trap_GPG_FindPair(TGPGroup GPG, const char *key)
{
	return (TGPValue)Q_syscall(GPG_FIND_PAIR, GPG, key);
}

qboolean trap_GPG_FindPairValue(TGPGroup GPG, const char *key, const char *defaultVal, char *Value)
{
	return (qboolean)Q_syscall(GPG_FIND_PAIRVALUE, GPG, key, defaultVal, Value);
}
...
{
	return (qboolean)Q_syscall(GPV_GET_NAME, GPV, Value);
}

TGPValue trap_GPV_GetNext(TGPValue GPV)
{
	return (TGPValue)Q_syscall(GPV_GET_NEXT, GPV);
}

TGPValue trap_GPV_GetInOrderNext(TGPValue GPV)
{
	return (TGPValue)Q_syscall(GPV_GET_INORDER_NEXT, GPV);
}

TGPValue trap_GPV_GetInOrderPrevious(TGPValue GPV)
{
	return (TGPValue)Q_syscall(GPV_GET_INORDER_PREVIOUS, GPV);
}

qboolean trap_GPV_IsList(TGPValue GPV)
{
	return (qboolean)Q_syscall(GPV_IS_LIST, GPV);
}

qboolean trap_GPV_GetTopValue(TGPValue GPV, char *Value)
{
	return (qboolean)Q_syscall(GPV_GET_TOP_VALUE, GPV, Value);
}

TGPValue trap_GPV_GetList(TGPValue GPV)
{
	return (TGPValue)Q_syscall(GPV_GET_LIST, GPV);
}

void trap_CM_TM_Create(int terrainID)
{
	Q_syscall(CG_CM_TM_CREATE, terrainID);
}

void trap_CM_TM_AddBuilding(int x, int y, int side)
{
	Q_syscall(CG_CM_TM_ADDBUILDING, x, y, side);
}

void trap_CM_TM_AddSpot(int x, int y)
{
	Q_syscall(CG_CM_TM_ADDSPOT, x, y);
}

void trap_CM_TM_AddTarget(int x, int y, float radius)
{
	Q_syscall(CG_CM_TM_ADDTARGET, x, y, PASSFLOAT(radius));
}

void trap_CM_TM_Upload(const vec3_t origin, const vec3_t angle)
{
	Q_syscall(CG_CM_TM_UPLOAD, origin, angle);
}

void trap_CM_TM_ConvertPosition(void)
{
	Q_syscall(CG_CM_TM_CONVERT_POS);
}
...
{
	return (void *)Q_syscall( CG_VM_LOCALALLOC, size );
}

void *trap_VM_LocalAllocUnaligned ( int size )
{
	return (void *)Q_syscall( CG_VM_LOCALALLOCUNALIGNED, size );
}

void *trap_VM_LocalTempAlloc( int size )
{
	return (void *)Q_syscall( CG_VM_LOCALTEMPALLOC, size );
}

void trap_VM_LocalTempFree( int size )
{
	Q_syscall( CG_VM_LOCALTEMPFREE, size );
}

const char *trap_VM_LocalStringAlloc ( const char *source )
{
	return (const char *)Q_syscall( CG_VM_LOCALSTRINGALLOC, source );
}

void trap_UI_CloseAll ( void )
{
	Q_syscall( CG_UI_CLOSEALL );
}
	
void trap_UI_SetActiveMenu ( int menu )
{
	Q_syscall( CG_UI_SETACTIVEMENU, menu );
}
