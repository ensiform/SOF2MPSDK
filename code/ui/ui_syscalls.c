...
void trap_Cvar_Register( vmCvar_t *cvar, const char *var_name, const char *value, int flags, float MinValue, float MaxValue ) {
	Q_syscall( UI_CVAR_REGISTER, cvar, var_name, value, flags, PASSFLOAT(MinValue), PASSFLOAT(MaxValue) );
}
...
qhandle_t trap_R_RegisterFont(const char *fontName) {
	return (qhandle_t)Q_syscall( UI_R_REGISTERFONT, fontName );
}
...
void trap_R_DrawStretchPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const float* color, qhandle_t hShader ) {
	Q_syscall( UI_R_DRAWSTRETCHPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), color, hShader );
}
...
{
	return Q_syscall( UI_R_GETTEXTWIDTH, text, font, PASSFLOAT(scale), limit );
}
	
int		trap_R_GetTextHeight ( const char* text, qhandle_t font, float scale, int limit )
{
	return Q_syscall( UI_R_GETTEXTHEIGHT, text, font, PASSFLOAT(scale), limit );
}

void	trap_R_DrawText ( int x, int y, qhandle_t font, float scale, vec4_t color, const char* text, int limit, int flags )
{
	Q_syscall( UI_R_DRAWTEXT, x, y, font, PASSFLOAT(scale), color, text, limit, flags );
}
...
{
	Q_syscall( UI_R_DRAWTEXTWITHCURSOR, x, y, font, PASSFLOAT(scale), color, text, limit, flags, cursorPos, cursor );
}

void trap_UpdateScreen( qboolean IsLoading ) {
	Q_syscall( UI_UPDATESCREEN, IsLoading );
}
...
{
	return Q_syscall( UI_VERIFY_CDKEY, key );
}
...
{
	return (int) (Q_syscall(UI_G2_ADDBOLT, ghoul2, modelIndex, boneName));
}

qboolean trap_G2API_RemoveBolt(void *ghlInfo, const int modelIndex, const int index)
{
	return (qboolean)(Q_syscall(UI_G2_REMOVEBOLT, ghlInfo, modelIndex, index));
}

qboolean trap_G2API_AttachG2Model(void *ghoul2From, int modelFrom, void *ghoul2To, int toBoltIndex, int toModel)
{
	return (qboolean)(Q_syscall(UI_G2_ATTACHG2MODEL, ghoul2From, modelFrom, ghoul2To, toBoltIndex, toModel));
}
...
{
	return (char *)Q_syscall(UI_G2_GETGLANAME, ghoul2, modelIndex);
}
...
{
	return Q_syscall(UI_G2_COPYSPECIFICGHOUL2MODEL, g2From, modelFrom, g2To, modelTo);
}
...
{
	return (qboolean)Q_syscall(UI_GPG_GET_NAME, GPG, Value);
}

TGPGroup trap_GPG_GetNext(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_NEXT, GPG);
}

TGPGroup trap_GPG_GetInOrderNext(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_INORDER_NEXT, GPG);
}

TGPGroup trap_GPG_GetInOrderPrevious(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_INORDER_PREVIOUS, GPG);
}

TGPGroup trap_GPG_GetPairs(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_PAIRS, GPG);
}

TGPGroup trap_GPG_GetInOrderPairs(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_INORDER_PAIRS, GPG);
}

TGPGroup trap_GPG_GetSubGroups(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_SUBGROUPS, GPG);
}

TGPGroup trap_GPG_GetInOrderSubGroups(TGPGroup GPG)
{
	return (TGPGroup)Q_syscall(UI_GPG_GET_INORDER_SUBGROUPS, GPG);
}

TGPValue trap_GPG_FindSubGroup(TGPGroup GPG, const char *name)
{
	return (TGPValue)Q_syscall(UI_GPG_FIND_SUBGROUP, GPG, name);
}

TGPValue trap_GPG_FindPair(TGPGroup GPG, const char *key)
{
	return (TGPValue)Q_syscall(UI_GPG_FIND_PAIR, GPG, key);
}

qboolean trap_GPG_FindPairValue(TGPGroup GPG, const char *key, const char *defaultVal, char *Value)
{
	return (qboolean)Q_syscall(UI_GPG_FIND_PAIRVALUE, GPG, key, defaultVal, Value);
}
...
{
	return (qboolean)Q_syscall(UI_GPV_GET_NAME, GPV, Value);
}

TGPValue trap_GPV_GetNext(TGPValue GPV)
{
	return (TGPValue)Q_syscall(UI_GPV_GET_NEXT, GPV);
}

TGPValue trap_GPV_GetInOrderNext(TGPValue GPV)
{
	return (TGPValue)Q_syscall(UI_GPV_GET_INORDER_NEXT, GPV);
}

TGPValue trap_GPV_GetInOrderPrevious(TGPValue GPV)
{
	return (TGPValue)Q_syscall(UI_GPV_GET_INORDER_PREVIOUS, GPV);
}

qboolean trap_GPV_IsList(TGPValue GPV)
{
	return (qboolean)Q_syscall(UI_GPV_IS_LIST, GPV);
}

qboolean trap_GPV_GetTopValue(TGPValue GPV, char *Value)
{
	return (qboolean)Q_syscall(UI_GPV_GET_TOP_VALUE, GPV, Value);
}

TGPValue trap_GPV_GetList(TGPValue GPV)
{
	return (TGPValue)Q_syscall(UI_GPV_GET_LIST, GPV);
}

void trap_Parental_Update ( void )
{
	Q_syscall ( UI_PARENTAL_UPDATE );
}

void trap_Parental_SetPassword ( const char* password )
{
	Q_syscall ( UI_PARENTAL_SET_PASSWORD, password );
}

void trap_Parental_GetPassword ( char* password, int buffersize )
{
	Q_syscall( UI_PARENTAL_GET_PASSWORD, password, buffersize );
}

void *trap_VM_LocalAlloc ( int size )
{
	return (void *)Q_syscall( UI_VM_LOCALALLOC, size );
}

void *trap_VM_LocalAllocUnaligned ( int size )
{
	return (void *)Q_syscall( UI_VM_LOCALALLOCUNALIGNED, size );
}

void *trap_VM_LocalTempAlloc( int size )
{
	return (void *)Q_syscall( UI_VM_LOCALTEMPALLOC, size );
}

void trap_VM_LocalTempFree( int size )
{
	Q_syscall( UI_VM_LOCALTEMPFREE, size );
}

const char *trap_VM_LocalStringAlloc ( const char *source )
{
	return (const char *)Q_syscall( UI_VM_LOCALSTRINGALLOC, source );
}

qboolean trap_NET_Available(void)
{
	return (qboolean)Q_syscall( UI_NET_AVAILABLE );
}

void trap_Version_GetDescription(char *description, int length)
{
	Q_syscall(UI_VERSION_GET_DESCRIPTION, description, length);
}

int trap_Version_GetNumSites(void)
{
	return (int)Q_syscall(UI_VERSION_GET_NUM_SITES);
}

void trap_Version_GetSite(int index, char *site, int length)
{
	Q_syscall(UI_VERSION_GET_SITE, index, site, length);
}

void trap_Version_Download(int index)
{
	Q_syscall(UI_VERSION_DOWNLOAD, index);
}

void trap_PunkBuster_Enable ( void )
{
	Q_syscall( UI_PB_ENABLE );
}

void trap_PunkBuster_Disable ( void )
{
	Q_syscall( UI_PB_DISABLE );
}

int trap_PunkBuster_IsEnabled ( void )
{
	return Q_syscall( UI_PB_ISENABLED );
}

int trap_GetTeamCount ( int team )
{
	return Q_syscall( UI_GET_TEAM_COUNT, team );
}

int trap_GetTeamScore ( int team )
{
	return Q_syscall( UI_GET_TEAM_SCORE, team );
}
